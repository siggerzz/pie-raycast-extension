...
// GitHub doesn't natively support PKCE, so we need to use the Raycast proxy. Ideally we would set up our own OAuth application with custom scopes, but this won't currently work. By default, we set all the same scopes as the GitHub extension to prevent access token issues.
